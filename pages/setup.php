...
if (rex::getUser()->isAdmin()) {
    ...
    foreach ($modules_dir as $mod_name) {
        ...
        $content .= '<p><a class="btn btn-primary '.($is_installed ? '' : 'btn-save').'" href="index.php?page=buchungskalender/setup&amp;install_module='.md5($mod_name).'" class="rex-button">Modul '.$mod_name.' '.($is_installed ? 'aktualisieren' : 'installieren') .'</a></p>';
    }
    ...
    foreach ($etpl_dir as $etpl_name) {
        ...
        $content .= '<p><a class="btn btn-primary '.($is_installed ? '' : 'btn-save').'" href="index.php?page=buchungskalender/setup&amp;install_etpl='.$etpl_name.'" class="rex-button">Template '.$etpl_name.' '.($is_installed ? 'aktualisieren' : 'installieren') .'</a></p>';
    }
    
    

    $fragment = new rex_fragment();
    $fragment->setVar('title', $this->i18n('buchungskalender_setup'), false);
